...
import { ImageBackground, Dimensions, Platform, Image, TouchableWithoutFeedback, KeyboardAvoidingView, Linking, Text, TouchableOpacity } from 'react-native'
import { Bubble, GiftedChat, SystemMessage } from '../../../lib/react-native-gifted-chat'
...
export default class GiftedChatModule extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      question: (this.props.navigation.state && this.props.navigation.state.params && this.props.navigation.state.params.question)
    }
    _isMounted = false
    socket = SocketIOClient(`${BASE_URL}?id=` + this.props.screenProps.user.user._id);
  ...
  fetchData = (skip, limit) => {
    ...
    if (!netStatus) {
      ...
      fetchApi.getChatList(getChatListLink, this.state.question._id, skip, limit, this.props.screenProps.user.user.accessToken)
        .then(result => {
  ...
  onLoadEarlier = () => {
    ...
    this.setState({ isLoadingEarlier: true, skip: this.state.messages.length }, () => {
      ...
      if (!netStatus) {
        ...
        fetchApi.getChatList(getChatListLink, this.state.question._id, this.state.skip, this.state.limit, this.props.screenProps.user.user.accessToken)
          .then(result => {
            ...
            if (result.status == 200) {
              ...
              this.setState((previousState) => {
                return {
                  messages: GiftedChat.prepend(previousState.messages, newArray),
  ...
  onSend = (messages = []) => {
    ...
    socketMessage._id = uuid.v4(),
      ...
      socketMessage.user = {
        ...
        name: this.props.screenProps.user.user.firstName + ' ' + this.props.screenProps.user.user.lastName,
        avatar: (this.props.screenProps.user.user && this.props.screenProps.user.user.profileImage && this.props.screenProps.user.user.profileImage.thumbnail) ?
      ...
      socketMessage.receiverId = this.state.question.questionFrom != this.props.screenProps.user.user._id ? this.state.question.questionFrom : this.state.question.questionTo
  ...
  renderCustomView(props) {
    ...
    return (
      <View>
        <CustomView {...props} />
        <Text style={{ paddingHorizontal: 16, paddingVertical: 8, color: colors.primary }}>{moment(props.currentMessage.createdAt).format('L') + ' @ ' + moment(props.currentMessage.createdAt).format('LT')}</Text>
      </View>)
  }
  ...
  onSendFromUser = (messages = []) => {
    ...
    if (!netStatus) {
      ...
      fetchApi.uploadImage(uploadImageLink, messages.fileUrl, this.props.screenProps.user.user.accessToken)
        .then(result => {
          ...
          if (result.status == 200) {
            ...
            if (messages.messageType == "IMAGE") {
              socketMessage = {
                ...
                receiverId: this.state.question.questionFrom != this.props.screenProps.user.user._id ? this.state.question.questionFrom : this.state.question.questionTo,
                // user,
            ...
            socket.emit('sendMessage', socketMessage, (response) => {
              if (response.status == 200) {
                this.setState(previousState => ({
                  messages: GiftedChat.append(previousState.messages, socketMessage),
                  step
                }))
              }
            })
  ...
  reactSocket = (reaction, messages) => {
    debugger
    let socketMessage = {}
    messages.currentMessage.reactions[reaction] = !messages.currentMessage.reactions[reaction]
    socketMessage._id = uuid.v4(),
      // socketMessage.createdAt= new Date(Milliseconds),
  ...
  renderBubble = props => {
    ...
    return (
      <View>
        ...
        />
        {(props.currentMessage.user._id != this.props.screenProps.user.user._id) ?
          <View style={{ flexDirection: 'row', marginVertical: 16, marginHorizontal: 8, justifyContent: (props.position == "right") ? "flex-end" : 'flex-start' }}>
            {props.currentMessage.reactions.love ?
              <TouchableWithoutFeedback onPress={() => this.reactSocket("love", props)}>
                <Image source={Images.ic_heart_Filled} />
              </TouchableWithoutFeedback> : props.currentMessage.reactions.happy ? <TouchableWithoutFeedback onPress={() => this.reactSocket("happy", props)}>
                <Image source={Images.ic_laughemoji_Filled} />
              </TouchableWithoutFeedback> : props.currentMessage.reactions.notClear ? <TouchableWithoutFeedback onPress={() => this.reactSocket("notClear", props)}>
                <Image source={Images.ic_questionmarkFilled} />
              </TouchableWithoutFeedback> :
                  <View style={{ flexDirection: 'row', justifyContent: (props.position == "right") ? "flex-end" : 'flex-start' }}>
                    <TouchableWithoutFeedback onPress={() => this.reactSocket("love", props)}>
                      <Image source={Images.ic_heart} />
                    </TouchableWithoutFeedback>
                    <TouchableWithoutFeedback onPress={() => this.reactSocket("happy", props)}>
                      <Image source={Images.ic_laughemoji} style={{ marginHorizontal: 16 }} />
                    </TouchableWithoutFeedback>
                    <TouchableWithoutFeedback onPress={() => this.reactSocket("notClear", props)}>
      ...
      <SafeAreaViewCustome>
        <View style={{ flex: 1 }}>
          ...
          <View style={{ padding: 16 }}>
            <TextCustom p style={{ color: colors.primary, marginBottom: 8 }}>{questionObject.day}</TextCustom>
            <TextCustom h5 style={{ color: colors.primary }}>{this.state.question.text.trim()}</TextCustom>
          </View>
          <GiftedChat
