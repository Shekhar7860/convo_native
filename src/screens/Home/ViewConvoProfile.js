...
export default class ViewConvoProfile extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      chatWindow: [{ name: string("Createyourownquestionfor") }],
      questions: []
    };
  }
  componentDidMount() {
  ...
  fetchData(skip, limit) {
    ...
    if (!netStatus) {
      ...
      // setIndicator(true)
      let convoId =
        this.state.data && this.state.data._id ? this.state.data._id : "";
      fetchApi
        .getQuestionList(
          getAddQuestionlink,
          convoId,
        ...
        .then(res => {
          ...
          if (res.status == 200) {
            ...
            if (skip == 0) {
              this.setState({
                questions: res.success.data,
                count: res.success.count,
                indicator1: false
              });
            } else {
              this.setState({
                questions: this.state.questions.concat(res.success.data),
                count: res.success.count,
                indicator1: false
              });
            }
  ...
  addQuestion = () => {
    ...
    if (validation.length != 0) {
      ...
      if (!netStatus) {
        ...
        data["convoId"] =
          this.state.data && this.state.data && this.state.data._id
            ? this.state.data._id
            : "";
        data["questionTo"] =
          this.state.data &&
          this.state.data.convoTo &&
          this.state.data.convoTo._id
            ? this.state.data.convoTo._id
            : "";
        fetchApi
          .addQuestion(
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text h4 style={{ color: colors.white, marginTop: 16 }}>
                {string("Createyourownquestionfor")}
              </Text>
            </View>
            <View style={{ alignItems: "center", zIndex: 1 }}>
              <Image
                source={
                  this.state.data.convoTo &&
                  this.state.data.convoTo.profileImage &&
                  this.state.data.convoTo.profileImage.thumbnail
                    ? { uri: this.state.data.convoTo.profileImage.thumbnail }
                    : Images.ic_userplaceholder
                }
                style={{
                  height: 130,
                  width: 130,
            ...
            >
              ...
              >
                {this.state.data.convoTo.firstName +
                  this.state.data.convoTo.lastName}
              </Text>
              <TextInput
                style={{
            ...
            >
              ...
              >
                <Text h4 style={{ color: colors.primary, fontSize: 16 }}>
                  {string("submit")}
                </Text>
              </TouchableOpacity>
            </View>
  ...
  chatWindowList = ({ item, index }) => {
    return (
      <TouchableOpacity
        onPress={() =>
          this.props.navigation.navigate("GiftedChatModule", { question: item })
        }
        style={{
          paddingVertical: 8,
      ...
      >
        <Text h4 style={{ color: colors.primary, marginRight: 8 }}>
          {item.text.trim()}
        </Text>
  ...
  questionList = ({ item, index }) => {
    ...
    return (
      <View>
        {userId == item.questionFrom || !item.unAnsweredQuestion ? (
          <View />
        ) : (
          <View style={{ alignItems: "center", paddingVertical: 12 }}>
            ...
            <TouchableOpacity
              onPress={() =>
                userId == item.questionFrom
                  ? null
                  : this.sendAnswer(item._id, this.state.answer[index], index)
              }
              style={{
                marginTop: -17,
            ...
            >
              <Text h4 style={{ color: colors.white, fontSize: 16 }}>
                {string("saveanswer")}
              </Text>
            </TouchableOpacity>
        ...
        >
          <ScrollView>
            <View style={{ flex: 1, alignItems: "center" }}>
              ...
              <Text p style={{ color: colors.grey3, fontSize: 14 }}>
                last responded on 8/12/19 @ 3:43pm
              </Text>
              <TouchableOpacity
